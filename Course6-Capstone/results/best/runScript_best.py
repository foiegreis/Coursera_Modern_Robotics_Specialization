...
class PickAndPlace(YouBot):
    ...
    def test_joint_limits(self, q, j):
        ...
        joint1_limit, joint2_limit, joint3_limit, joint4_limit, joint5_limit = self.joint_limits

        constraints = np.array([joint1_limit, joint2_limit,
                                joint3_limit, joint4_limit, joint5_limit,
                                0, 0, 0, 0])

        q_check = q[3:]
        j_check = j.copy()
        for i, qi in enumerate(q_check):
            if i == 0:  # joint 1 will stay inside +- joint1_limit
                if constraints[i] != 0 and (qi < constraints[i] or qi > -constraints[i]):
                    j_check[:, i] = 0
...
if __name__ == '__main__':
    ...
    temp = sys.stdout
    sys.stdout = open(f'log{suffix}.txt', 'w')
    ...
    df.to_csv(f'trajectory{suffix}.csv', header=False, index=False)
    df = pd.DataFrame(x_err_list)
    df.to_csv(f'err{suffix}.csv', header=False, index=False)
    df = pd.DataFrame(u_list)
    df.to_csv(f'controls{suffix}.csv', header=False, index=False)
    df = pd.DataFrame(planned_trajectory)
    df.to_csv(f'desired_trajectory{suffix}.csv', header=False, index=False)
    ...
    controller.plot_and_save_error(f'err{suffix}.csv', f'err{suffix}.png')
    controller.plot_and_save_controls(f'controls{suffix}.csv', f'controls{suffix}.png')

    print(f"Coppeliasim path         {os.path.join(os.path.dirname(os.path.abspath(__file__)), f'trajectory{suffix}.csv')}")
